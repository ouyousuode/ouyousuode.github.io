...
<img src="/images/posts/2019-03-01/Figure_16_1_Kernel_filters.png">
如Figure 16.1所示，在内核内部是很多的过滤器。当内核工作时，它告诉这些过滤器正发生着什么。图中有一个过滤器用于进程处理，另一个用于网络栈，再一个则与文件系统相关，诸如此类。每一个过滤器都有一组它观测的感兴趣的事件，比如出现在某socket上的数据，一个程序已经**fork()**了，某信号已被发出了，又或者一个文件已添加到某目录！
